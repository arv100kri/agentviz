# FAX-VIZ UI/UX Style Guide

> The definitive design system reference for FAX-VIZ.
...
FAX-VIZ follows a "quiet power tool" aesthetic inspired by Linear, Raycast, and Vercel.
...
// Always renders: FAX-VIZ<span style={{ color: theme.accent.primary }}>.</span>
```
...
- Product name: **FAX-VIZ** (all caps, no spaces).
...
**FAX-VIZ uses zero CSS files for component styling.** All styles are inline `style={}` objects.
...
| `BrandWordmark` | "FAX-VIZ." logo with accent dot |
...
- [ ] **Brand**: Product name is "FAX-VIZ" (all caps). Uses `BrandWordmark` component.
